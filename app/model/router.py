...
async def get_iphone(id: str):  # -> List[SModels_iphone]
    iphone = await ModelsDAO.model_one_or_none(model_id=id)
    if iphone is None:
...
async def add_iphone(model_iphone: SModels_iphone):
    existing_iphone = await ModelsDAO.model_one_or_none(name=model_iphone.name)
    if existing_iphone:
...
async def search_shops_by_partial_name(name: str):
    models = await ModelsDAO.get_shops_by_partial_name(name)
    if not models:
        raise HTTPException(status_code=404, detail="Такого нет")
    return models
...
async def delete_iphone(id: str):
    await ModelsDAO.delete(model_id=id)
